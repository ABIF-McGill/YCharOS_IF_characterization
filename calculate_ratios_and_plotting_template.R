...
summary_ab_scene_data <- data %>%
  group_by(antibody, scene, celltype) %>%
...
summary_ab_data.wide <- dcast(summary_ab_data, antibody ~ celltype, value.var="med")
...
### remove ctrl and 0 rows
summary_ab_scene_data.wide_2 = subset(summary_ab_scene_data.wide_2, summary_ab_scene_data.wide_2$antibody != "0")
summary_ab_data_2 = subset(summary_ab_data_2, summary_ab_data_2$antibody != "0")



ggplot(summary_ab_data_2, aes(antibody, ratio)) +
  geom_col(color="dark grey",fill="white")+
  geom_jitter(data = summary_ab_scene_data.wide_2, alpha=0.4, width=0.1) +
  geom_text(aes(x = antibody, y = -0.1, label = round(ratio, digits = 1)), size=3)+
  theme_classic() +
  #ylim(-0.2,4) +
  xlab("")+
